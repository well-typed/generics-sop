...
--
-- This is the internal module that exports the internal representation
-- of n-ary products.
--
-- Users of this library should preferably use the 'Data.SOP.NP' module.
--
module Data.SOP.NP.Internal
  ( -- * Datatypes
    NP(.., Nil, (:*))
  , POP(..)
  , unPOP
...
-- This implies that using ':*' for construction is inefficient,
-- and functions that construct products should use the provided
-- higher-order functions if possible.
--
-- The type @NP@ is spine-strict.
--
...
type role NP representational representational
...
viewNP :: forall f xs . NP f xs -> ViewNP f xs
viewNP (NP xs)
  | null xs   =
    unsafeCoerce @(ViewNP f '[]) @(ViewNP f xs)
      $ IsNil
  | otherwise =
...
pattern x :* xs <- (viewNP -> IsCons x xs)
  where
    x :* NP xs = NP (V.cons (unsafeCoerce @(f _) @(f Any) x) xs)
infixr 5 :*
...
instance All (NFData `Compose` f) xs => NFData (NP f xs) where
  rnf np = unK (ccata_NP (Proxy @(NFData `Compose` f)) (K ()) (\ x (K r) -> K (x `seq` r)) np)
...
pure_NP f =
  NP (V.replicate (lengthSList (Proxy @xs)) f)
{-# INLINE pure_NP #-}
...
pure_POP f =
  POP (cpure_NP (allP topP) (pure_NP f))
{-# INLINE pure_POP #-}
...
fromList xs
  | length xs == lengthSList (Proxy @xs) = Just (NP (V.fromListN (lengthSList (Proxy @xs)) (coerce xs)))
  | otherwise                            = Nothing
...
ap_NP (NP fs) (NP xs) = NP (V.zipWith coerce fs xs)
...
projections =
  ana_NP
    (\ (K i) -> (Fn (\ (K (NP xs)) -> unsafeCoerce @(f Any) @(f _) (xs V.! i)), K (i + 1)))
    (K 0)
...
collapse_NP (NP xs) = coerce (V.toList xs)
...
ctraverse__NP p f =
  traverse__NP unK . cmap_NP p (K . f)
...
sequence'_NP (NP mxs) =
  NP <$> sequenceA (coerce mxs)
  -- NOTE: We cannot replace the outer application of the @NP@ constructor with
  -- coerce because of the pessimistic assumption about the role of @f@ ...
...
ctraverse'_NP p f np =
  traverse'_NP unComp (cmap_NP p (Comp . f) np)
...
cata_NP nil cons (NP xs) =
  V.foldr
    (unsafeCoerce @(f Any -> r xs -> r (_ : _)) @(f Any -> r xs -> r xs) cons)
    (unsafeCoerce @(r '[]) @(r xs) nil)
    xs
...
ana_NP uncons s =
  NP
    (V.unfoldrN
      ...
      (Just . unsafeCoerce @(s (_ : _) -> (f _, s _)) @(Any -> (f Any, Any)) uncons)
      (unsafeCoerce @(s xs) @Any s)
    )
...
cana_NP ::
  ...
  -> NP f xs
cana_NP p uncons s =
  NP (V.fromList (unK (apFn (go @xs) s)))
  where
    go :: forall xs' . All c xs' => (s -.-> K [f Any]) xs'
    go =
      cpara_SList
        p
        (Fn $ \ _ -> K [])
        (\ rec -> Fn (\ r -> case uncons r of (x, r') -> K (unsafeCoerce @(f _) @(f Any) x : unK (apFn rec r'))))
  -- NOTE: We could reduce 'cana_NP' to 'ana_NP', but we would introduce a
  -- mutual dependency then, because we still need a function such as 'cmap_NP'
  -- or 'cpure_NP', which currently depend on 'cana_NP'.
