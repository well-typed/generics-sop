...
-- | n-ary products (and products of products) as a type-family
module Data.SOP.NP.TF
{-
  ( -- * Datatypes
    NP(..)
  , POP(..)
...
--
data family NP :: (k -> Type) -> [k] -> Type
data instance NP f '[] = Nil
data instance NP f (x ': xs) = f x :* NP f xs
...
instance All (Show `Compose` f) xs => Show (NP f xs) where
  showsPrec = runShowsPrec $ cpara_SList (Proxy :: Proxy (Show `Compose` f)) nil cons where
    nil = ShowsPrec $ \_ _ -> showString "Nil"

    cons :: Show (f y) => ShowsPrec f ys -> ShowsPrec f (y ': ys)
    cons rec = ShowsPrec $ \d (f :* fs) -> showParen (d > 5)
      $ showsPrec (5 + 1) f
      . showString " :* "
      . runShowsPrec rec 5 fs

newtype ShowsPrec f xs = ShowsPrec { runShowsPrec :: Int -> NP f xs -> ShowS }
...
cpure_NP p f = cpara_SList p Nil (f :*) where
...
ap_NP = runApImpl (para_SList nil cons) where
    nil = ApImpl $ \_  _  -> Nil

    cons :: ApImpl f g ys -> ApImpl f g (y : ys)
    cons rec = ApImpl $ \(Fn f :* fs) (x :* xs) -> f x :* runApImpl rec fs xs

newtype ApImpl f g xs = ApImpl { runApImpl :: NP (f -.-> g) xs -> NP f xs -> NP g xs }
