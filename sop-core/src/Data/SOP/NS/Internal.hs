...
-- of n-ary sums.
--
-- Users of this library should preferably use the 'Data.SOP.NS' module.
--
module Data.SOP.NS.Internal
  ( -- * Datatypes
    NS(.., Z, S)
  , SOP(..)
  , unSOP
...
-- This implies that using 'S' for construction is inefficient,
-- and functions that construct sums should use the provided
-- higher-order functions if possible.
--
-- The type @NS@ is spine-strict.
--
-- /Examples:/
--
-- > Z         :: f x -> NS f (x ': xs)
...
type role NS representational representational
...
viewNS :: forall f xs . NS f xs -> ViewNS f xs
viewNS (NS i x)
...
pattern Z x <- (viewNS -> IsZ x)
  where
    Z x = NS 0 (unsafeCoerce @(f _) @(f Any) x)
...
instance All (Show `Compose` f) xs => Show (NS f xs) where
  showsPrec d (Z fx) =
    showParen (d > 10)
      (showString "Z " . showsPrec 11 fx)
  showsPrec d (S s) =
    showParen (d > 10)
      (showString "S " . showsPrec 11 s)
...
ejections =
  NP (V.generate (lengthSList (Proxy @xs)) (\ i -> Fn (\ (K (NS j fx)) -> Comp (if i == j then Just fx else Nothing))))
...
-- @since 0.5.1.0
emptyNS :: NS f '[] -> Void
emptyNS x = x `seq` error "emptyNS: impossible"
...
-- @since 0.2.2.0
--
unZ :: forall f x . NS f '[x] -> f x
unZ (NS _ x) = unsafeCoerce @(f Any) @(f _) x
...
injections =
  NP (V.generate (lengthSList (Proxy @xs)) (\ i -> Fn (\ fx -> K (NS i fx))))
...
ap_SOP (POP (NP nps)) (SOP (NS i fx)) = SOP (NS i (ap_NP (nps V.! i) fx))
...
ctraverse__NS p f =
  traverse__NS unK . cmap_NS p (K . f)
...
ctraverse'_NS p f ns =
  traverse'_NS unComp (cmap_NS p (Comp . f) ns)
...
cata_NS z s = go
  where
    ...
    go (Z x) = z x
    go (S i) = s (go i)
    -- NOTE: This is reasonably ok for the compact representation;
    -- we have to iterate the @s@ function several times anyway.
