...
-- | Generate @staged-sop@ boilerplate instances using Template Haskell.
module Generics.SOP.Staged.TH
  ( deriveGeneric
  , deriveGenericOnly
  , deriveGenericSubst
...
import qualified Generics.SOP.Staged as SOP
import Data.SOP.NP
import Data.SOP.NS
...
--
-- > instance SGeneric Tree where
-- >
-- >   type SDescription Tree = '[ '[Int], '[Tree, Tree] ]
-- >
-- >   sfrom (Leaf x)   = SOP (   Z (I x :* Nil))
-- >   sfrom (Node l r) = SOP (S (Z (I l :* I r :* Nil)))
-- >
...
deriveGenericSubst n f =
  deriveGenericOnlySubst n f
{-
  dec <- reifyDatatype n
...
deriveGenericFunctions n codeName fromName toName = do
  ...
  withDataDec dec $ \_variant _cxt name bndrs instTys cons -> do
    ...
    let repType  = [t| SOP SOP.C $(appTyVars varT codeName' bndrs) |] -- SOP C TreeCode
    sequence
...
deriveGenericForDataDec f _variant _cxt name _bndrs instTys cons = do
  ...
  deriveGenericForDataType f name typ cons

deriveGenericForDataType :: (Name -> Q Type) -> Name -> Q Type -> [TH.ConstructorInfo] -> Q [Dec]
deriveGenericForDataType f name typ cons = do
  let descriptionSyn = tySynInstDCompat ''SOP.SDescription Nothing [typ] (codeFor f cons)
  ufs <- uniqueFields cons
  c <- newName "c"
  let constraintsSyn = tySynInstDCompat ''SOP.Constraints Nothing [varT c, typ] (constraintsFor c ufs)
  c' <- newName "c"
  let ccon = mkName ("C" <> simplify name)
  let constraintsD = dataInstD (cxt []) ''SOP.ConstraintsD [varT c', typ] Nothing [normalC ccon (dictsFor c' ufs)] []
  inst <- instanceD
            (cxt [])
            [t| SOP.SGeneric $typ |]
            [ descriptionSyn             -- type SDescription ... = ...
            , constraintsSyn             -- type Constraints ... = ...
            , constraintsD               -- data ConstraintsD ... = ...
            , constraints 'SOP.constraints ccon ufs -- constraints = ...
            , embedding 'SOP.sfrom cons  -- sfrom ... = ...
            , projection 'SOP.sto cons   -- sto ... = ...
            , allC 'SOP.allC ccon ufs cons -- allC ... = ...
            ]
  return [inst]
...
uniqueFields :: [TH.ConstructorInfo] -> Q [Type]
uniqueFields cis = do
  ts <- concat . (snd <$>) <$> mapM conInfo cis
  ts' <- sequence ts
  pure (nub ts')

constraintsFor :: Name -> [Type] -> Q Type
constraintsFor c ufs = do
  constraintsT (appT (varT c) . pure <$> ufs)

dictsFor :: Name -> [Type] -> [Q BangType]
dictsFor c ufs =
  (\ t -> bangType (bang noSourceUnpackedness sourceStrict) [t| SOP.Dict $(varT c) $(pure t) |]) <$> ufs

constraints :: Name -> Name -> [Type] -> Q Dec
constraints cName ccName ufs = do
  let n = length ufs
  funD cName [clause [] (normalB (appsE (conE ccName : replicate n [| SOP.Dict |]))) []]

allC :: Name -> Name -> [Type] -> [TH.ConstructorInfo] -> Q Dec
allC cName ccName ufs cis = do
  c <- newName "c"
  ds <- mapM (const $ newName "d") ufs
  let
    n :: Int
    n = length ufs
    ...
    extract :: Int -> Q Exp
    extract i =
      typedBracketE $ caseE (typedSpliceE (varE c))
        [ do
            x <- newName "x"
            match (conP ccName (replicate i wildP <> [varP x] <> replicate (n - 1 - i) wildP)) (normalB (varE x)) []
        ]
    ...
    dds :: [Q Dec]
    dds = zipWith (\ i d -> funD d [clause [] (normalB [| Comp (SOP.C $(extract i)) |]) []]) [0 .. n - 1] ds
  funD cName [clause [varP c] (normalB (letE dds (popE pop))) []]
...
embedding fromName cis = funD fromName [cl] -- . go' (\e -> [| Z $e |])
  where
    cl :: Q Clause
    cl = do
      x <- newName "x"
      k <- newName "k"
      clause [varP x, varP k] (normalB (typedBracketE (caseE (typedSpliceE (varE x)) (makeMatches k (\ e -> [| Z $e |]) cis)))) []

    makeMatches :: Name -> (Q Exp -> Q Exp) -> [TH.ConstructorInfo] -> [Q Match]
    makeMatches _ _ [] = []
    makeMatches k br (ci : cis) =
      let
        m =
          do
            (n, ts) <- conInfo ci
            vars <- replicateM (length ts) (newName "y")
            match (conP n (map varP vars)) (normalB (typedSpliceE (appE (varE k) (appE [| SOP |] (br (npE (appE [| SOP.C |] . typedBracketE . varE <$> vars))))))) []
      in
        m : makeMatches k (\ e -> [| S $(br e) |]) cis
...
projection toName = funD toName . go'
  where
    ...
    mkClause br c = do
      (n, ts) <- conInfo c
      vars    <- replicateM (length ts) (newName "x")
      clause [conP 'SOP [br . conP 'Z . (:[]) . npP . map (\v -> conP 'SOP.C [varP v]) $ vars]]
             (normalB . typedBracketE . appsE $ conE n : map (typedSpliceE . varE) vars)
             []
...
constraintsT :: [Q Type] -> Q Type
constraintsT [c] = c
constraintsT cs  = foldl' appT (tupleT (length cs)) cs
...
simplify = reverse . takeWhile (/= '.') . reverse . show
